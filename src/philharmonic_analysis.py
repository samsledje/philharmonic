...
if __name__ == "__main__":
    ...
    logg.info('Filtering edges with confidence threshold {}...'.format(edge_weight_thresh))
    wG = nx.Graph()
    ...
    SC = SpectralClustering(n_clusters=k_clusts, assign_labels="discretize", random_state=random_state, affinity='precomputed')
    SC.fit(simDSD)
    ...
    while True:
        ...
        if csize <= max_cluster_size:
            break
        SC2 = SpectralClustering(n_clusters=2, assign_labels="discretize", random_state=random_state, affinity='precomputed')
