...
if __name__ == "__main__":
    ...
    parser.add_argument('--manual_annot_wt', type=float, help='Relative weight of proteins with manual annotation', default=100)
    ...
    for ap in allowed_proteins:
        protein_weight_df.append((ap, args.manual_annot_wt if ap in shortlisted_proteins else 1))
    protein_weight_df = pd.DataFrame(protein_weight_df, columns=['protein', 'wt'])

    l0 = list(range(len(protein_weight_df)))
    j_u = np.random.choice(l0, int(args.paircount), True, (protein_weight_df["wt"]/protein_weight_df["wt"].sum()).values)
    j_v = np.random.choice(l0, int(args.paircount), True, (protein_weight_df["wt"]/protein_weight_df["wt"].sum()).values)
