...
def extract_clusters(SC: SpectralClustering, og_clust: list, og_csize: int, verbosity=2):
    subCs = []
...
if __name__ == "__main__":
    ...
    parser.add_argument('--init_k', type=int, default=500, help='Initial number of clusters')
    parser.add_argument('--min_cluster_size', type=int, default=3, help='Minimum cluster size')
    parser.add_argument('--cluster_divisor', type=int, default=20, help='Cluster divisor')
    parser.add_argument('--sparsity', type=float, default=1e-5, help='Sparsity threshold')
    ...
    SC = SpectralClustering(n_clusters=args.init_k, assign_labels="discretize", random_state=args.random_seed, affinity='precomputed')
    SC.fit(sim)
    ...
    while True:
        ...
        n_clusters = int(np.round(csize / args.cluster_divisor)) # NOTE: this changed so that we are left with clusters of max size 30

        if n_clusters < 2:
            clustQ.put((priority, c))
            break
    
        SC2 = SpectralClustering(n_clusters=n_clusters, assign_labels="discretize", random_state=args.random_seed, affinity='precomputed')
        SC2.fit(sim[c,:][:,c])
    ...
    for cl in clustsNames:
        ...
        clustsDict[uqid] = {
            "members": members,
            "graph": [(i[0], i[1], subgraph.get_edge_data(i[0], i[1])["weight"]) for i in subgraph.edges()],
            }
